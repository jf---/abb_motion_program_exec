...
def rr_inertia_to_loaddata_abb(rr_inertia, rr_inertia_pose=None):
    ...
    if rr_inertia_pose is not None:
        ...
        aom = (
            rox.quatproduct(NamedArrayToArray(rr_inertia_pose[0]["orientation"])) @ aom
        )
    ...
    if not confdata_extra:
        wrist_vs_axis1 = (
            rox.fwdkin(
                ...
                [
                    0,
                    rr_robot_pose.joint_position_seed[1],
                    rr_robot_pose.joint_position_seed[2],
                    0,
                    0,
                    0,
                ],
        ...
        wrist_vs_lower_arm = (
            rox.fwdkin(
                cfx_robot, [0, 0, rr_robot_pose.joint_position_seed[2], 0, 0, 0]
            ).p[0]
            < 0
        )
        axis_5_sign = rr_robot_pose.joint_position_seed[4] < 0
        cfx = np.packbits(
            [axis_5_sign, wrist_vs_lower_arm, wrist_vs_axis1], bitorder="little"
        ).item()
        ...
        cd = abb_exec.confdata(
            confdata_extra.data[0],
            confdata_extra.data[1],
            confdata_extra.data[2],
            confdata_extra.data[3],
        )
...
class MoveAbsJCommandConv:
    ...
    freeform_names = [
        "MoveAbsJ",
        "MoveAbsJCommand",
        "experimental.robotics.motion_program.MoveAbsJCommand",
    ]

    def apply_rr_command(self, cmd, mp, **kwargs):
        zd = rr_zone_to_abb(
            cmd_get_arg(cmd, "fine_point"), cmd_get_arg(cmd, "blend_radius")
        )
        sd = rr_speed_to_abb(cmd_get_arg(cmd, "tcp_velocity"))
        jt = rr_joints_to_abb(
            cmd_get_arg(cmd, "joint_position"), cmd_get_arg(cmd, "joint_units", [])
        )
...
class MoveJCommandConv:
    ...
    freeform_names = [
        "MoveJ",
        "MoveJCommand",
        "experimental.robotics.motion_program.MoveJCommand",
    ]
    ...
    def apply_rr_command(self, cmd, mp, cfx_robot, **kwargs):
        ...
        sd = rr_speed_to_abb(cmd_get_arg(cmd, "tcp_velocity"))
        rt = rr_robot_pose_to_abb(
            cmd_get_arg(cmd, "tcp_pose"), cfx_robot, cmd_get_extended(cmd, "confdata")
        )
        mp.MoveJ(rt, sd, zd)
...
class MoveCCommandConv:
    ...
    freeform_names = [
        "MoveC",
        "MoveCCommand",
        "experimental.robotics.motion_program.MoveCCommand",
    ]
    ...
    def apply_rr_command(self, cmd, mp, cfx_robot, **kwargs):
        ...
        rt2 = rr_robot_pose_to_abb(
            cmd_get_arg(cmd, "tcp_via_pose"),
            cfx_robot,
            cmd_get_extended(cmd, "confdata_via"),
        )
...
class WaitTimeCommandConv:
    ...
    freeform_names = [
        "WaitTime",
        "WaitTimeCommand",
        "experimental.robotics.motion_program.WaitTimeCommand",
    ]
...
class SetToolCommandConv:
    ...
    freeform_names = [
        "SetTool",
        "SetToolCommand",
        "experimental.robotics.motion_program.SetToolCommand",
    ]
...
class SetPayloadCommandConv:
    ...
    freeform_names = [
        "SetPayload",
        "SetPayloadCommand",
        "experimental.robotics.motion_program.SetPayloadCommand",
    ]
    ...
    def add_setup_args(self, cmd, setup_args):
        abb_payload = rr_payload_to_abb(
            cmd_get_arg(cmd, "payload_info"), cmd_get_arg(cmd, "payload_pose")
        )
...
class CirPathModeCommandConv:
    ...
    freeform_names = [
        "CirPathMode",
        "CirPathModeCommand",
        "experimental.abb_robot.motion_program.CirPathModeCommand",
    ]
...
class SyncMoveOnCommandConv:
    ...
    freeform_names = [
        "SyncMoveOn",
        "SyncMoveOnCommand",
        "experimental.abb_robot.motion_program.SyncMoveOnCommand",
    ]
...
class SyncMoveOffCommandConv:
    ...
    freeform_names = [
        "SyncMoveOff",
        "SyncMoveOffCommand",
        "experimental.abb_robot.motion_program.SyncMoveOffCommand",
    ]
...
class EGMStreamConfigCommandConv:
    ...
    freeform_names = [
        "EGMStreamConfig",
        "EGMStreamConfigCommand",
        "experimental.abb_robot.motion_program.EGMStreamConfigCommand",
    ]
...
class EGMJointTargetConfigCommandConv:
    ...
    freeform_names = [
        ...
        "EGMJointTargetConfigCommand",
        "experimental.abb_robot.motion_program.EGMJointTargetConfigCommand",
    ]
    ...
    def add_setup_args(self, cmd, setup_args):
        ...
        setup_args["egm_config"] = abb_exec.EGMJointTargetConfig(
            J1, J2, J3, J4, J5, J6, max_position_deviation, max_speed_deviation
        )
...
class EGMPoseTargetConfigCommandConv:
    ...
    freeform_names = [
        "EGMPoseTargetConfig",
        "EGMPoseTargetConfigCommand",
        "experimental.abb_robot.motion_program.EGMPoseTargetConfigCommand",
    ]
    ...
    def add_setup_args(self, cmd, setup_args):
        ...
        max_position_deviation = cmd_get_arg(cmd, "max_position_deviation")
        max_speed_deviation = cmd_get_arg(cmd, "max_speed_deviation")
        setup_args["egm_config"] = abb_exec.EGMPoseTargetConfig(
            corr_frame,
            corr_fr_type,
            sensor_frame,
            sensor_fr_type,
...
class EGMPathCorrectionConfigCommandConv:
    ...
    freeform_names = [
        ...
        "EGMPathCorrectionConfigCommand",
        "experimental.abb_robot.motion_program.EGMPathCorrectionConfigCommand",
    ]
...
class EGMRunJointCommandConv:
    ...
    freeform_names = [
        "EGMRunJoint",
        "EGMRunJointCommand",
        "experimental.abb_robot.motion_program.EGMRunJointCommand",
    ]
...
class EGMRunPoseCommandConv:
    ...
    freeform_names = [
        "EGMRunPose",
        "EGMRunPoseCommand",
        "experimental.abb_robot.motion_program.EGMRunPoseCommand",
    ]
...
class EGMMoveLCommandConv:
    ...
    freeform_names = [
        "EGMMoveL",
        "EGMMoveLCommand",
        "experimental.abb_robot.motion_program.EGMMoveLCommand",
    ]
...
class EGMMoveCCommandConv:
    ...
    freeform_names = [
        "EGMMoveC",
        "EGMMoveCCommand",
        "experimental.abb_robot.motion_program.EGMMoveCCommand",
    ]
...
class SetWorkObjectCommandConv:
    ...
    freeform_names = [
        "SetWorkObject",
        "SetWorkObjectCommand",
        "experimental.abb_robot.motion_program.SetWorkObjectCommand",
    ]
    ...
    if cmd.datatype == "experimental.robotics.motion_program.FreeformCommand":
        ...
        if conv is None:
            if cmd.data.optional:
                raise OptionalCommandException(
                    f"Optional command {cmd.data.command_name}"
                )
            else:
...
def get_rr_motion_program_task(rr_mp, default_task="T_ROB1"):
    ...
    if groups.datatype == "string":
        ...
        if groups.data[0].datatype == "string":
            ...
            groups.data[0].datatype == "int32[]"
            or groups.data[0].datatype == "uint32[]"
        ):
