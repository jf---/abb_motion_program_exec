...
class MotionExecImpl:
    def __init__(self, mp_robot_info, base_url, username, password):
        ...
        self._abb_client = abb_client.MotionProgramExecClient(
        ...
        try:
            ...
            for chain_i in range(len(self.robot_info.chains)):
                self._rox_robots.append(
                    self._robot_util.robot_info_to_rox_robot(self.robot_info, chain_i)
                )
        except:
            traceback.print_exc()
            raise ValueError(
                "invalid robot_info, could not populate GeneralRoboticsToolbox.Robot"
            )
    ...
    def execute_motion_program(self, program, queue):
        ...
        abb_program, is_multimove, tasks = rr_motion_program_to_abb2(
            program, self._rox_robots
        )

        gen = ExecuteMotionProgramGen(
            self, self._abb_client, abb_program, is_multimove, tasks
        )
...
class ExecuteMotionProgramGen(SyncTaskGenerator):
    def __init__(
        ...
        super().__init__(
            RRN,
            RRN.GetStructureType(
                "experimental.robotics.motion_program.MotionProgramStatus"
            ),
            1,
            -1,
        )
    ...
    def RunTask(self):
        ...
        if not self._is_multimove:
            robot_recording_data = self._abb_client.execute_motion_program(
                self._motion_program, task=self._tasks
            )
        else:
            robot_recording_data = self._abb_client.execute_multimove_motion_program(
                self._motion_program, tasks=self._tasks
            )
...
class RobotRecordingGen:
    def __init__(
        ...
        self._mp_recording_part = RRN.GetStructureType(
            "experimental.robotics.motion_program.MotionProgramRecordingPart"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="ABB Robot motion program driver service for Robot Raconteur"
    )
    parser.add_argument(
        "--mp-robot-info-file",
        ...
        default=None,
        required=True,
        help="Motion program robot info file (required)",
    )
    parser.add_argument(
        "--mp-robot-base-url",
        type=str,
        default="http://127.0.0.1:80",
        help="robot controller ws base url (default http://127.0.0.1:80)",
    )
    parser.add_argument(
        "--mp-robot-username",
        type=str,
        default="Default User",
        help="robot controller username (default 'Default User')",
    )
    parser.add_argument(
        "--mp-robot-password",
        type=str,
        default="robotics",
        help="robot controller password (default 'robotics')",
    )

    args, _ = parser.parse_known_args()

    RRC.RegisterStdRobDefServiceTypes(RRN)
    register_service_types_from_resources(
        RRN,
        __package__,
        [
            "experimental.robotics.motion_program",
            "experimental.abb_robot.motion_program",
        ],
    )
    ...
    with args.mp_robot_info_file:
        ...
        if "robot_info" not in mp_robot_info_dict:
            d = {"robot_info": mp_robot_info_dict}
            mp_robot_info_dict = d

    info_loader = InfoFileLoader(RRN)
    mp_robot_info, mp_robot_ident_fd = info_loader.LoadInfoFileFromDict(
        mp_robot_info_dict,
        "experimental.robotics.motion_program.MotionProgramRobotInfo",
        "mp_robot",
    )

    attributes_util = AttributesUtil(RRN)
    mp_robot_attributes = attributes_util.GetDefaultServiceAttributesFromDeviceInfo(
        mp_robot_info.robot_info.device_info
    )

    mp_exec_obj = MotionExecImpl(
        mp_robot_info,
        args.mp_robot_base_url,
        args.mp_robot_username,
        args.mp_robot_password,
    )

    with RR.ServerNodeSetup(
        "experimental.robotics.motion_program", 59843, argv=sys.argv
    ):
