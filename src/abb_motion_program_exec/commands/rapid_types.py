...
Types mirror the ABB Robot Controller's RAPID language. Documentation is taken from the
...
class speeddata(NamedTuple):
    ...
    The ``abb_motion_program_exec`` module contains constants for ``v5`` through ``v5000``
    and ``vmax`` as found in RAPID.
    """
    ...
    def to_rapid(self) -> str:
        return util.nums_to_rapid_array(
            [self.v_tcp, self.v_ori, self.v_leax, self.v_reax]
        )
...
class zonedata(NamedTuple):
    ...
    The ``abb_motion_program_exec`` module contains constants for ``fine`` and ``z0``
    through ``z200`` as found in RAPID.
    """
...
class loaddata(NamedTuple):
    ...
    def to_rapid(self) -> str:
        return (
            f"[{self.mass},{util.nums_to_rapid_array(self.cog)},"
...
class wobjdata(NamedTuple):
    ...
    def to_rapid(self) -> str:
        return (
            f"[{util.bool_to_rapid(self.robhold)},{util.bool_to_rapid(self.ufprog)},"
