...
class MotionProgramExecClient:
    """Client to execute motion programs on an ABB IRC5 controller using Robot Web Services (RWS)."""
    ...
    def execute_motion_program(
        ...
        filename, b = _get_motion_program_file(
            self.abb_client.get_ramdisk_path(), motion_program, task, seqno=seqno
        )
    ...
    def _download_and_start_motion_program(
        ...
        if exec_state.ctrlexecstate != "stopped":
            raise RuntimeError("Controller must be stopped to execute motion program")
        ctrl_state = self.abb_client.get_controller_state()
        if ctrl_state != "motoron":
            raise RuntimeError("Controller must be motoron to execute motion program")
